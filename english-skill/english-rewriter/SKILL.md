...
For every question (the text to rewrite), provide rewritten versions across the following six distinct registers, in this order:
...
**Non-English lines**: only rewrite lines containing English text; lines with no English text (e.g. lines consisting only of Chinese characters, code fences, or punctuation) are kept as-is, unchanged, in all six versions.
