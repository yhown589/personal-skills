...
1. A **question block** starts at a heading line whose text contains a timestamp in the format `YYYY-MM-DD HH:mm:ss.SSS` (e.g. `# 1 2026-07-10 18:18:04.830`) and extends until the first `<blockquote` tag, the next such heading line, or the end of file — whichever comes first. Everything between the heading and that terminator is the block's content. The heading line itself is a marker, not question text: the question is the English text in the block's content (normally a single line holding one Part 3 question). Content not belonging to any heading-started block is ignored.
...
1. Read the `ielts-speaking-part3` skill definition: it is the base skill of this batch skill, located in the sibling folder named `ielts-speaking-part3` (this batch skill's folder name without the `-batch` suffix) under the same skills directory — read that folder's `SKILL.md` (once, before processing any question).
...
https://example.com/part3-may-2025
</blockquote>
